...
##
# SUB Routine
func undoSubCleanFlags(from:Vector2i, to:Vector2i, hist:HistoryEntry):
	var selfSide = SIDES.WHITE if isWhiteTurn else SIDES.BLACK;
	...
	if(hist._getPromote()):
		...
		bbClearIndexOf(HexEngine.QRToIndex(to.x, to.y,), isWhiteTurn, hist._getPPieceType());
		
		activePieces[selfSide][hist._getPPieceType()].pop_back();
		activePieces[selfSide][PIECES.PAWN].insert( hist._getPIndex(), Vector2i(from) );
		
		unpromoteValid = true;
		unpromoteIndex = hist._getPIndex();
		@warning_ignore("int_as_enum_without_cast") unpromoteType = hist._getPPieceType();
	
	if(hist._getCapture()):
		if(hist._getIsCaptureTopSneak()):
			to.y += 1 if isWhiteTurn else -1;
		bbAddPieceOf(HexEngine.QRToIndex(to.x, to.y,), !isWhiteTurn, hist._getCPieceType());
		activePieces[SIDES.BLACK if isWhiteTurn else SIDES.WHITE][hist._getCPieceType()].insert( hist._getCIndex(), Vector2i(to) )
			
		uncaptureValid = true;
		captureIndex = hist._getCIndex();
		@warning_ignore("int_as_enum_without_cast") captureType = hist._getCPieceType();
...
func undoSubFixState() -> void:
	...
	var history:HistoryEntry = historyStack[historyStack.size()-1];
	...
	if(history._getEnPassant()):
		EnPassantCordsValid = true;
		var from:Vector2i = history._getFrom();
		var to:Vector2i = history._getTo();
...
func _undoLastMove(genMoves:bool=true) -> bool:
	...
	var activeMove:HistoryEntry = historyStack.pop_back();
	...
	if(genMoves): generateNextLegalMoves();
	return true;
